...
export interface CacheOptions {
  ...
  capacity?: number;
  deleteOnExpire?: string; // 'none' | 'passive' | 'aggressive'
  enable?: boolean;
  maxAge?: number;
  onExpire?: Function;
  recycleFreq?: number;
  storageMode?: string; // 'memory' | 'localStorage' | 'sessionStorage'
...
export class CacheFactory {
   ...
   */
  createCache(id, options: CacheOptions = {}) {
    if (this.caches[id]) {
      throw new Error(`cache "${id}" already exists!`);
    }
    options['parent'] = this;
    this.caches[id] = new Cache(id, options);
